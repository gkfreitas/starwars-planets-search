...
const mockContext = {
  planets: mockPlanets,
  filterBy: { column: 'population', comparison: 'maior que', value: '0' },
  filters: [],
  nameFilter: '',
  setFilterBy: jest.fn(),
  loadPlanets: jest.fn(),
...
test('Teste app', async () => {
  render(
  <PlanetsContext.Provider value={ mockContext }>
    <App />
  ...
  const tables = screen.getAllByRole('row')
  ...
  userEvent.type(numberFilter, '500');
  ...
  expect(tables).toHaveLength(11);
  const removerALlFilters = screen.getByRole('button', {
    name: /remover todas filtragens/i
  })
  expect(removerALlFilters).toBeInTheDocument();
